...
// ----------------------------------------------------------------------------
Solution Rbpp::solve(Data const &data, int const &maxDeviations, First_Fit Ff, First_Fit inutile) {


	// upper bound on the number of bins
	int U = Ff.sol_init.size();
	...
	try {
		...
		for(int i = 0; i < n; i++){
			x[i] = IloNumVarArray(env, U, 0, 1, ILOINT);
		}
		IloNumVarArray y(env, U, 0, 1, ILOINT);
